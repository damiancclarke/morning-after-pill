# BirthsEstimates13.R v2.00        KEL / DCC               yyyy-mm-dd:2013-12-29
#---|----1----|----2----|----3----|----4----|----5----|----6----|----7----|----8
#
...
# ifications, augmenting to include birth data for an additional year released
# recently.
#
#
...
#=== (3) Load Data
#==============================================================================
f <- paste(brth.dir, "S1Data_granular_covars13.csv", sep="")
orig <- read.csv(f)
...
datcollapse <- function(age_sub,order_sub,ver,dat) {
    ...
    dat <- closegen(0,4,dat)
    dat <- closegen(4,8,dat)
    dat <- closegen(8,12,dat)

    dat$close10 <- dat$close4
    dat$close20 <- dat$close8
    dat$close30 <- dat$close12
    dat$road10  <- dat$road4
    dat$road20  <- dat$road8
    dat$road30  <- dat$road12
    dat$time10  <- dat$time4
    dat$time20  <- dat$time8
    dat$time30  <- dat$time12
...
event <- function(age_sub,order_sub,logm) {
    ...
    formod$pilln7[formod$add==7 & formod$treatCom==1]   <- 1
    formod$pilln7[is.na(formod$pilln7)]                 <- 0
    formod$pilln6[formod$add==6 & formod$treatCom==1]   <- 1
    formod$pilln6[is.na(formod$pilln6)]                 <- 0
    ...
    formod$pillp3[formod$pill==1 & formod$pilltotal==4] <- 1
    formod$pillp3[is.na(formod$pillp3)]                 <- 0
    f1 <- formod

    eventS <- lm(f1$births ~ factor(f1$year) + factor(f1$dom_comuna) +
                 factor(f1$dom_comuna):f1$trend                      + 
                 factor(f1$pilln6) + factor(f1$pilln5)               +
    ...
    return(list("b" = beta, "s" = se, "eventyr" = c(-6,-5,-4,-2,-1,0,1,2,3)))
}
...
eventPlot(e1519,"Event1519-w13.eps",-10,5)
eventPlot(e2034,"Event2034-w13.eps",-10,5)
eventPlot(eAll ,"EventAll-w13.eps" ,-5,2.5)
...
to <-file(paste(tab.dir,"aggregateASFR-w13.tex", sep=""))
